...
4. Ще додамо зчитування файлів формату JSON, для API, поки у файл `posts/destroy.php`. [Тут приклад такої реалізації](https://i.imgur.com/QOuZiCu.png)
   - Тому виходячи з цього коду
   ```php
   $api_data = json_decode(file_get_contents('php://input'), 1);
   $data = $api_data ?? $_POST;
   ```
   - Я можу створити подібний функціонал і на створення постів
   - Тобто я іду у файл `blog\core\func.php` де є функція `validateInputsWithForm` де я і вкажу код, який буде приймати дані з `API`, якщо вони прийшли саме звідти
   - [Приклад створення постів з API, якщо запит прийшов з відти](https://i.imgur.com/4JZ5VBM.png)
   - Ну і якщо відправлю запит з POSTMAN на ствоерння поста, ми отримаємо [успішно створений пост](https://i.imgur.com/qJFBYkh.png)
   - А для відповіді про успішність самого запиту, ми також додамо вивід інформації через змінну `$res`, та перевірку на запит від [`API`](https://i.imgur.com/soySWXi.png)
